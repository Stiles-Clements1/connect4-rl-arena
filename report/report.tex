...
began near the supervised Zan convolutional baseline while the replay
...
networks (the convolutional and Transformer networks contributed by
the Stiles, Luke, and Zan teammates, including the original frozen
copy of $M_1$ but excluding the $M_1$ trained in this project);
...
$75.5\%$ across all evaluated opponents. Notably it defeated the Zan
convolutional network that warm-started its policy on $57\%$ of games,
...
\begin{table}[h]
  ...
  \begin{tabular}{lr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Soft Actor-Critic head-to-head win rates. The six Project-1 baselines comprise the two pretrained networks (one convolutional, one Transformer) contributed by each of the three teammate groups; this includes the original frozen copy of $M_1$ but not the $M_1$ trained in this project.}
  \label{tab:sac_eval}
\end{table}
